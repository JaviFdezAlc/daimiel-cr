...
function createTrip(overrides: Partial<TripSearchItem> = {}): TripSearchItem {
  return {
    id: 1,
    departureTime: "08:00",
    arrivalTime: "08:30",
...
describe("filterTrips", () => {
  it("filtra por fecha y numero minimo de plazas", () => {
    ...
    const result = filterTrips({
      trips: [availableTrip, insufficientSeatsTrip, differentDayTrip],
      dayOffset: 0,
      minSeats: 2,
      verifiedOnly: false,
      sort: "earliest",
  ...
  it("ordena los viajes por mayor numero de plazas sin mutar la lista original", () => {
    const trips = [
      createTrip({ id: 1, seats: 2 }),
      createTrip({ id: 2, seats: 4 }),
      createTrip({ id: 3, seats: 3 }),
    ];
